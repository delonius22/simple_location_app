...
def visualize(data_df: pd.DataFrame, save_path="user_location_visualization.html"):
    ...
    Args:
        data_df: DataFrame containing login data with latitude, longitude, timestamp,
                user_id, device_id, is_anomalous, and anomaly_score columns
        save_path: Path to save the HTML visualization file
    ...
    for idx, row in data_df.iterrows():
        # Format timestamp for TimestampedGeoJson - THE KEY FIX IS HERE
        # TimestampedGeoJson expects ISO format strings
        ...
        feature = {
            ...
            'geometry': {
                'type': 'Point',
                'coordinates': [float(row['longitude']), float(row['latitude'])]  # Ensure float type
            },
            'properties': {
                'time': time_str,
                'icon': 'circle',
    ...
    try:
        time_layer = TimestampedGeoJson(
            data=time_data,  # Explicitly use 'data' parameter name
            period='PT1H',
            duration='PT10M',
            transition_time=200,
            auto_play=False,
        ...
        print(f"Error creating TimestampedGeoJson layer: {e}")
        # Continue without the time layer if it fails
    ...
    # Save the map - Add error handling
    try:
        m.save(save_path)
